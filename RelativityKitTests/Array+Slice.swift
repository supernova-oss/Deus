...
extension Array {
  /// Produces an array containing the elements at the given `bounds` of this
  /// one.
  ///
  /// - Parameter indices: Indices of this ``Array`` at which the elements to be
  ///   copied into the produced one are.
  /// - Returns: Another ``Array`` with `indices.count` elements of this one, or
  ///   this one itself in case the specified ``indices`` equal to those of this
  ///   ``Array``.
