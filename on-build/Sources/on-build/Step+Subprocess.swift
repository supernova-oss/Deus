...
extension Executable {
  ...
  /// Executable at `usr/bin/swift-format`, relative to the directory of the
  /// Swift toolchain.
  static var swiftFormat: Self {
    get async throws(StepError) {
      ...
      guard
        ...
        throw .unexpectedOutput(executable: swiftly, arguments: arguments)
      }
...
extension Step {
  /// Spawns a subprocess and executes the given executable, passing the given
  /// arguments in and with the ``projectURL`` as the working directory. The
  /// output is forwarded to the standard output and ignored; to capture it,
  /// call ``spawnSubprocessAndCapture(_:_:)``.
  ///
  /// - Parameters:
  ...
  /// Spawns a subprocess and executes the given executable, passing the given
  /// arguments in and with the ``projectURL`` as the working directory. The
  /// output is captured and returned by this function; to forward it to the
  /// standard output, call ``spawnSubprocess(_:_:)``.
  ///
  /// - Parameters:
  ///   - executable: The executable to execute.
  ///   - arguments: The arguments to pass to the executable.
  func spawnSubprocessAndCapture(
    ...
    guard
      ...
      throw .unexpectedOutput(executable: executable, arguments: arguments)
    }
    return output
  }
}
...
/// Spawns a subprocess and executes the given executable, passing the given
/// arguments in and with the ``projectURL`` as the working directory.
///
/// - Parameters:
///   - workingDirectory: Directory to which every path is relative, from which
///     the executable will be executed.
///   - executable: The executable to execute.
///   - arguments: The arguments to pass to the executable.
