...
extension Duration {
  /// Whether an integer amount of ticks can be performed by a ``Clock`` within
  /// this `Duration`.
  ...
  var comprisableSubtickCount: Double {
    .init(attoseconds) / .init(Self.subtickScale)
  }
  ...
  /// Makes a `Duration` within which a ``Clock`` can perform subticks the
  /// specified amount of times.
  ///
  /// - Parameter count: Quantity of subticks comprised by the `Duration`.
  static func subticks(_ count: some BinaryInteger) -> Self {
    .microseconds(count)
  }
}
