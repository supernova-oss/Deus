...
/// Bézier curve whose endpoints are controlled by one control ``Point`` each;
/// consists, therefore, of four points.
///
/// ## Formula
///
/// B(*t*) = (1 - *t*)³P₀ + 3(1 - *t*)²*t*P₁ + 3(1 - *t*)*t*²P₂ + *t*³P₃, where
/// 0 ≤ *t* ≤ 1
private struct CubicBezierCurve: BezierCurveProtocol {
  ...
  subscript(_ t: Double) -> Point {
    ...
    return (p0 * pow(y, 3)) as Point + p1 * 3 * pow(y, 2) * t + p2 * 3 * y
      * pow(t, 2) + p3 * pow(t, 3)
  }
}

/// Property of non-linear Bézier curves (i.e., quadratic, cubic and
/// higher-degree ones) that comprises a controlled ``Point`` and another one
/// which controls it: its control ``Point``. The first is positioned on the
/// curve in relation to the latter (which is not, itself, part of the curve).
public struct Control: Sendable {
  ...
  /// ``Point`` which influences the ``controllee`` by arching the curve toward
  /// itself.
  ...
  ///
  /// - Parameters:
  ///   - controller: ``Point`` which influences the ``controllee`` by arching
  ///     the curve toward itself.
  ///   - controllee: Anchor or interpolation ``Point`` controlled by the
  ///     ``controller``.
...
extension Point {
  /// Defines the given ``Point`` as the control ``Point`` of this one in a
  /// Bézier curve.
  ///
  /// - Parameter controller: ``Point`` which influences this one by arching the
  ///   curve toward itself.
...
extension BezierCurveProtocol {
  /// Performs an assertion when in debug configuration that guarantees that
  /// ``t`` is ≥ 0 and ≤ 1.
  ///
  /// Should be called by all implementations of ``subscript(_:)`` prior to any
  /// calculation.
  ///
...
public struct BezierCurve {
  ...
  public static func make(
    from start: Control,
    to end: Control
  ) -> some BezierCurveProtocol { CubicBezierCurve(start: start, end: end) }
}
