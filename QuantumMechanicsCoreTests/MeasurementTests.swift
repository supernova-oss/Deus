...
fileprivate struct AngleTests {
  ...
  func adds(_ addition: Angle, to initial: Angle) {
    #expect(
      (initial + addition).quantityInBaseUnit
  ...
  func subtracts(_ subtraction: Angle, from initial: Angle) {
    #expect(
      (initial - subtraction).quantityInBaseUnit
        .isApproximatelyEqual(
  ...
  func isDescribedByQuantityInCurrentUnitAndSymbol(
    _ unitRepresentable: UnitRepresentable<Angle>
  ) {
    let angle = Angle.in(unitRepresentable.unit)(2)
    #expect(
      "\(angle)"
        == "\(Angle.formatted(quantity: angle.quantityInCurrentUnit)) \(angle.symbol)"
    )
  }
  ...
  @Test(
    arguments: zip(
      [Angle.radians(2)],
      [UnitRepresentable(unit: \Angle.Type.radians)]
    )
  )
  func converts(
    _ initial: Angle,
    into unitRepresentable: UnitRepresentable<Angle>
  ) {
...
fileprivate struct ElectricChargeTests {
  ...
  func makes(in unitRepresentable: UnitRepresentable<ElectricCharge>) {
    ...
    #expect(
      electricCharge.conversionCoefficient == measurable.conversionCoefficient
    )
  ...
  func negates(in unitRepresentable: UnitRepresentable<ElectricCharge>) {
    ...
    #expect(
      negatedElectricCharge == ElectricCharge.in(unitRepresentable.unit)(-2)
    )
    #expect(negatedElectricCharge.symbol == initialElectricCharge.symbol)
  }

  @Test(arguments: [
    (ElectricCharge.elementary(2), ElectricCharge.elementary(2))
  ])
  ...
  func converts(
    ...
    #expect(converted.quantityInBaseUnit == initial.quantityInBaseUnit)
    #expect(
      converted.symbol == ElectricCharge.in(unitRepresentable.unit).symbol
    )
  }
}
...
fileprivate struct MassTests {
  ...
  @Test(arguments: [
    (
      Mass.electronvoltsPerLightSpeedSquared(2),
      Mass.electronvoltsPerLightSpeedSquared(2)
    ),
    (
      .electronvoltsPerLightSpeedSquared(2),
  ...
  func isDescribedByQuantityInCurrentUnitAndSymbol(
    _ unitRepresentable: UnitRepresentable<Mass>
  ) {
    let mass = Mass.in(unitRepresentable.unit)(2)
    #expect(
      "\(mass)"
        == "\(Mass.formatted(quantity: mass.quantityInCurrentUnit)) \(mass.symbol)"
    )
  }
  ...
  func converts(
    _ initial: Mass,
    into unitRepresentable: UnitRepresentable<Mass>
  ) {
    let converted = initial.converted(into: unitRepresentable.unit)
    #expect(converted.quantityInBaseUnit == initial.quantityInBaseUnit)
...
fileprivate struct SpeedTests {
  ...
  @Test(arguments: [UnitRepresentable(unit: \Speed.Type.metersPerSecond)])
  func isDescribedByQuantityInCurrentUnitAndSymbol(
    _ unitRepresentable: UnitRepresentable<Speed>
  ) {
    let speed = Speed.in(unitRepresentable.unit)(2)
    #expect(
      "\(speed)"
        == "\(Speed.formatted(quantity: speed.quantityInCurrentUnit)) \(speed.symbol)"
    )
  }
  ...
  func converts(
    _ initial: Speed,
    into unitRepresentable: UnitRepresentable<Speed>
  ) {
    let converted = initial.converted(into: unitRepresentable.unit)
    #expect(converted.quantityInBaseUnit == initial.quantityInBaseUnit)
...
/// Representation of an SI unit in which a quantity of a ``Measurement`` may
/// be. Exists merely for debugging purposes, allowing for displaying the actual
/// name of the unit while testing rather than the string of the key path.
