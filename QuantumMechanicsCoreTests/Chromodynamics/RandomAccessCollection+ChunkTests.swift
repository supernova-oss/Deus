...
struct RandomAccessCollectionChunkTests {
  ...
  func chunkingWithAZeroedOrNegativeSizeReturnsAnEmptyArray(
    size: Int,
    allowsPartiality: Bool
  ) {
    #expect(
      [2, 4].chunked(into: size, allowsPartiality: allowsPartiality) == []
    )
  }
  ...
  {
    #expect(
      [2, 4].chunked(into: size, allowsPartiality: allowsPartiality) == [[2, 4]]
    )
  }
