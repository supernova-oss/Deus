...
/// Heaviest ``Quark``, with a Lagrangian mass of 173.21 ± 0.51 ± 0.7 GeV/*c*².
/// Decays to a ``BottomQuark``.
...
/// Second heaviest ``Quark``, with a Lagrangian mass of 4.18 ± 0.03 GeV/*c*².
/// Decays to a ``CharmQuark``.
...
/// Third heaviest ``Quark``, with a Lagrangian mass of 1.275 ± 0.025 GeV/*c*².
/// Decays to a ``StrangeQuark``.
...
/// Third lightest ``Quark``, with a Lagrangian mass of 95 ± 5 MeV/*c*². Decays
/// to a ``DownQuark``.
...
/// Second lightest ``Quark``, with a Lagrangian mass of 4.8 ± 0.5 ± 0.3
/// MeV/*c*². Decays to an ``UpQuark``.
...
/// Lightest ``Quark``, with a Lagrangian mass of 2.3 ± 0.7 ± 0.5 MeV/*c*². As
/// per the Standard Model, cannot decay.
...
public struct AnyQuark: Discrete, Quark {
  ...
  public static let discretion = AnySingleColor.discretion
    .flatMap { color in
      [
        Self.init(UpQuark(colorLike: color)),
        .init(DownQuark(colorLike: color)), .init(CharmQuark(colorLike: color)),
        .init(StrangeQuark(colorLike: color)),
  ...
  public init(_ base: some Quark) {
    if let base = base as? Self {
      self = base
    } else if let base = base as? AnyQuarkLike,
      let color = base.colorLike as? any SingleColor
    {
      self.base = base.base
      spin = base.spin
      charge = base.charge
...
/// A quark (q) is an elementary fermion ``ColoredParticle`` which is confined,
/// bound to at least another one by gluon ``Particle``s via strong force. It is
/// the only ``Particle`` in the Standard Model which experiences each of the
...
///
/// Ordinary matter, such as nucleons and, therefore, atoms, is composed by 1ˢᵗ
/// generation quarks due to their stability: they have a decay width Γ ≈ 0 and,
/// consequently, a lifetime τ ≈ ∞, because them being the lightest ones either
/// diminishes the chances of (d) or prohibits (u) their decay to lighter —
/// unknown or nonexistent — ``Particle``s of the ``Color`` field. Scenarios
/// beyond the Standard Model in which up quarks decay are those of Grand
/// Unification Theories, which theorize that the aforementioned four
/// fundamental forces were one in the very early Universe (first picosecond of
/// its existence) and, as described by the theory of proton decay by Andrei
...
public protocol QuarkLike: ColoredParticleLike
where ColorLike: SingleColorLike {}
