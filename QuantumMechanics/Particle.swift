...
/// Subatomic excitation of an underlying field, which exhibits discrete
/// interactions with other fields. Differs from vacuum fluctuations or
/// delocalized modes on such field in that it is localized: it has a wavepacket
/// of location. Such localization is made possible by collapse-like,
/// interpretation-defined consequence of measuring its wavefunction
/// Ψ(*x*, *t*), where *x* is the position and *t* is the time.
///
/// ## Classification
///
...
public protocol ParticleLike: Comparable, Sendable {
  ...
  /// Force experienced by this type of ``ParticleLike``s in an electromagnetic
  /// field.
  var charge: ElectricCharge { get }

  /// Character which identifies this type of ``ParticleLike`` as per the
  /// International System of Units (SI).
  var symbol: String { get }

  /// Performs a partial equality comparison between this ``ParticleLike`` and
  /// the given one by testing all their properties common to such protocol
  /// against each other.
  ///
  /// ## Partiality
  ///
  /// This method returning `true` does not necessarily denote that both
  /// ``ParticleLike``s are equal; rather, as the name implies, the existing
  /// equality is partial, meaning that properties that are specific to either
  /// `Self` or the type of `other` may differ and, therefore, still
  /// differentiate them from one another.
  ///
  /// Some ``ParticleLike``s might resort to their type-specific equality
  /// comparison, falling back to the partial comparison in case the first deems
  /// their structure different (e.g., an equatable ``ParticleLike`` initially
  /// tests `self == other`; if `false`, then the partial comparison is
  /// performed).
  ///
  /// ## Implicit contract with equatable particle-likes
  ///
  /// Comparing two ``ParticleLike``s of the same type which conform to the
  /// `Equatable` protocol by calling this method should always return `true`
  /// when `self == other`. However, because of the aforementioned
  /// considerations on partiality,
  /// `isPartiallyEqual(to: other) && self == other` may be `false`.
  ///
  /// - Parameter other: ``ParticleLike`` to which this one will be compared.
  /// - Returns: `true` if the properties shared by these ``ParticleLike``
...
extension ParticleLike {
  ...
  ///
  /// This function is distinct from the public one in that it allows for
  /// overriders which employ type-specific checks to resort to the default
  /// check in case such type-specific checks return `false`.
  ///
  /// - Parameter other: ``ParticleLike`` to which this one will be compared.
  /// - Returns: `true` if the properties shared by these ``ParticleLike``
  ///   values are equal; otherwise, `false`.
