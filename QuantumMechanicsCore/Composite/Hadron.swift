...
/// ``Particle`` composed by two or more ``Quark``s which are bound by strong
/// force. It is the compositor of nucleons — such as protons and neutrons —
/// and, therefore, the most common composite ``Particle`` in the universe.
///
/// ## Classification
///
...
///
/// > Glossary: • **Baryon number**: ⅓ of the difference between the number of
///               quarks and that of antiquarks. Because of ``Color``
///               confinement, a phenomenon in which ``ColoredParticle``s are
///               confined, hadrons cannot have a net ``Color`` charge; thus, a
///               hadron being composed of three ``Quark``s of distinct
///               ``Color``s (a baryon) or of a ``Quark`` and an antiquark (a
///               meson) has a ``Color``-neutral state — it is white;\
///             • **q̄**: antiquark, the antiparticle of a ``Quark``.\
///             • **⊗**: tensor product of two vector spaces *V* and *W*: an
///               *m* × *n* matrix, where *m* is the number of components of
///               *V* and *n* is that of *W*.
public protocol Hadron: ColoredParticle {
  associatedtype ColorLike = White

  /// ``Quark``s by which this ``Hadron`` is composed, bound by strong force via
  /// the gluon ``Particle``s.
...
extension Hadron where Self: ColoredParticleLike {
  public var charge: ElectricCharge {
    ...
    for quark in quarks {
      quantityInBaseUnit += quark.charge.quantityInBaseUnit
    }
