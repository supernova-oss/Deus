...
public protocol ColorLike: Equatable, Sendable {
  ...
  ///
  /// Erasure is disregarded: by calling this function on or passing in an
  /// any-``ColorLike``, such check is performed against the original, unerased
  /// type of such erased ``ColorLike`` instead of that of its wrapper.
  ///
...
/// field, SU(3)₍color₎ or gluon field, is quantized as gluons, boson
/// ``Particle``s responsible for binding the ``Quark``s of a ``Hadron`` by
...
/// - be effectless.
///
/// > Note: None of the two mentioned concepts, color and direction, refer to
///   their classical description (respectively, a visual perception of the
///   electromagnetic spectrum and a projection of physical movement from one
///   point toward another). These are uniquely-quantum properties of a
///   ``ColoredParticle``.
