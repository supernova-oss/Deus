...
/// Base protocol to which ``ColoredParticle``s and colored antiparticles
/// conform.
...
extension ColoredParticleLike {
  ...
  ///
  /// - Parameter other: ``ColoredParticleLike`` to which this one will be
  ///   compared.
  /// - Returns: `true` if the properties shared by these
  ///   ``ColoredParticleLike`` values are equal; otherwise, `false`.
  func _coloredParticleLikeIsPartiallyEqual(
    to other: some ColoredParticleLike
  ) -> Bool {
    return _particleLikeIsPartiallyEqual(to: other)
      && (colorLike.is(Red.self) && other.colorLike.is(Red.self)
        ...
        || colorLike.is(Anti<Green>.self)
          && other.colorLike.is(Anti<Green>.self)
...
/// Direct (in the case of a gluon ``Particle``) or indirect result of a
/// localized excitation of the ``Color`` field.
public protocol ColoredParticle: ColoredParticleLike, Particle
where ColorLike: Color {}
